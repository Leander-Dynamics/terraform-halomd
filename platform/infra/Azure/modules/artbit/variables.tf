...
variable "enable_key_vault_private_endpoint" {
  type        = bool
  description = "Enable a private endpoint for Key Vault and disable public network access."
  default     = false
}
variable "vault_dns_zone_name" {
  type        = string
  description = "Private DNS zone name that contains Key Vault privatelink records."
