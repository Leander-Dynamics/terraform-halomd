...
resource "azurerm_service_plan" "arbit_workflow_application_frontend_app_serviceplan_1" {
...
resource "azurerm_linux_web_app" "arbit_workflow_application_frontend" {
  name                                           = "${var.env_region}-arbit-workflow-application-frontend"
  location                                       = azurerm_resource_group.workflow_rg.location
  resource_group_name                            = azurerm_resource_group.workflow_rg.name
  service_plan_id                                = azurerm_service_plan.arbit_workflow_application_frontend_app_serviceplan_1.id
...
resource "azurerm_private_endpoint" "workflow_frontend_private_ep" {
  ...
  location            = azurerm_linux_web_app.arbit_workflow_application_frontend.location
  resource_group_name = azurerm_linux_web_app.arbit_workflow_application_frontend.resource_group_name
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_linux_web_app.arbit_workflow_application_frontend.id
...
resource "azurerm_private_dns_a_record" "frontend_record" {
  name                = azurerm_linux_web_app.arbit_workflow_application_frontend.name
