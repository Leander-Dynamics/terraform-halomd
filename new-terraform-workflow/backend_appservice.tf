...
resource "azurerm_service_plan" "arbit-workflow-application-backend-app-serviceplan-1" {
...
# the backend app
resource "azurerm_linux_web_app" "arbit-workflow-application-backend" {
  name                                           = "${var.env_region}-arbit-workflow-application-backend"
  location                                       = azurerm_resource_group.workflow_rg.location
  resource_group_name                            = azurerm_resource_group.workflow_rg.name
  service_plan_id                                = azurerm_service_plan.arbit-workflow-application-backend-app-serviceplan-1.id
...
resource "azurerm_private_endpoint" "workflow_backend_private_ep" {
  ...
  location            = azurerm_linux_web_app.arbit-workflow-application-backend.location
  resource_group_name = azurerm_linux_web_app.arbit-workflow-application-backend.resource_group_name
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_linux_web_app.arbit-workflow-application-backend.id
...
resource "azurerm_private_dns_a_record" "backend_record" {
  name                = azurerm_linux_web_app.arbit-workflow-application-backend.name
